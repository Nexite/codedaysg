...
export default function CodeDayRegistrationForm({ language = 'en-US', webname }: Props) {
  const ref = useRef<HTMLIFrameElement>(null);
  const [height, setHeight] = useState<number>(400);
  
  // Validate that the message came from CodeDay
  const onResizeMessageHandler = useCallback((e: MessageEvent) => {
  ...
  useEffect(() => {
    ...
    ref.current?.addEventListener('load', () => ref.current?.contentWindow?.postMessage('poll', CODEDAY_ORIGIN));
